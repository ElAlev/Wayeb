...
object LogicUtils extends LazyLogging {
  ...
  def parsed2ActualSentence(logicSentence: LogicSentence): Sentence = {
    parsed2ActualSentence(logicSentence, Set.empty)
  }

  def parsed2ActualSentence(
                             logicSentence: LogicSentence,
    ...
    logicSentence match {
      case x: LogicAtomicSentence => {
        ...
        AtomicSentence(pred, registerSelection)
      }
      case x: LogicComplexSentence => {
        ComplexSentence(x.op, x.sentences.map(s => parsed2ActualSentence(s, registerSelection)))
      }
    }
  }
