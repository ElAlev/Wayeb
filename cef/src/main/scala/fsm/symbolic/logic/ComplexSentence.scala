...
case class ComplexSentence(
  ...
  override def evaluate(
    ...
    case NOT => !sentences.head.evaluate(event, valuation)
    case AND => sentences.forall(s => s.evaluate(event, valuation))
    case OR => sentences.exists(s => s.evaluate(event, valuation))
  }
   ...
   */
  override def getRegisterSelection: Set[String] = {
    val rs = sentences.flatMap(s => s.getRegisterSelection).toSet
    rs
  }
