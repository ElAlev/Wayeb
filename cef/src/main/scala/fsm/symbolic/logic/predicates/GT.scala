...
/**
  * Checks if the value of a certain event attribute (must be Double) is above a given constant.
  * @param arguments 0 is the attribute name to be checked
  *                  1 the given constant
  */
case class GT(override val arguments: List[String]) extends Predicate(arguments) {
  ...
  override def toString: String = "GT(" + list2Str(arguments, ",") + ")"
