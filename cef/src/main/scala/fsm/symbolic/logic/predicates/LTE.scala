...
/**
  * Checks if the value of a certain event attribute (must be Double) is below or equal to a given constant.
  * @param arguments 0 is the attribute name to be checked
  *                  1 the given constant
  */
case class LTE(override val arguments: List[String]) extends Predicate(arguments) {
  ...
  override def toString: String = "LTE(" + list2Str(arguments, ",") + ")"
